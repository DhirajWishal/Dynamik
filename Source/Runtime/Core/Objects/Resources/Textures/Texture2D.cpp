...
namespace Dynamik
{
	DMKFormat getFormat(gli::format format)
	{
		switch (format)
		{
	...
	void DMKTexture2D::_load_KTX(const STRING& path)
	{
		gli::texture texture = gli::load_ktx(path);
		if (!texture.data())
		...
		DMKMemoryFunctions::copyData(image, texture.data(), size() * layerCount);
	}
}
