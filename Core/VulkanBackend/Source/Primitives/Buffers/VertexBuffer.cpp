...
namespace DMK
{
	...
	{
		void VertexBuffer::Initialize(const VulkanDevice& vDevice, UI64 size)
		{
			vBuffer = CreateBuffer(vDevice, size, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT);
		...
		void* VertexBuffer::MapMemory(const VulkanDevice& vDevice, UI64 size, UI64 offset)
		{
		...
		void VertexBuffer::UnmapMemory(const VulkanDevice& vDevice)
		{
