...
std::vector<DMKMeshObject> DMKMeshImporter::loadMeshes(const STRING& path, const DMKVertexLayout& vertexLayout)
{
	AssimpWrapper _wrapper;
	return _wrapper.loadStaticModelEntity(path, vertexLayout).meshObjects;
}

void DMKMeshImporter::unloadMesh(const std::vector<DMKMeshObject>& meshes)
{
