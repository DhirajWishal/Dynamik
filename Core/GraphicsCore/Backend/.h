...
namespace DMK
{
	namespace GraphicsCore
	{
		...
		public:
			bool operator==(const RenderTargetAttachmentHandle& handle) const
			{
				return mHandle == handle.mHandle && mType == handle.mType;
			}
		};
		...
		public:
			...
			std::vector<RenderTargetAttachmentHandle> mAttachmentHandles;	// Attachment handles.
		...
		public:
			bool operator==(const RenderTargetHandle& handle) const
			{
				return mAttachmentHandles == handle.mAttachmentHandles
					&& mRenderPassID == handle.mRenderPassID
					&& mFrameBufferID == handle.mFrameBufferID
					&& mRenderTargetID == handle.mRenderTargetID;
			}
		};
	}
}